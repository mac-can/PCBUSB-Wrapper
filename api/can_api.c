 ...
 *
 *	compiler  :  Microsoft Visual C/C++ Compiler (Version 14.0)
 *	             Apple LLVM version 8.1.0 (clang-802.0.42)
 *
 *	export    :  (see header file)
 *
 *	includes  :  can_io.h (can_defs.h), PCANBasic.h
 *
...
#ifdef _DEBUG
static char _id[] = "CAN API V3 for PEAK PCAN-Basic Interfaces, Version 3.2 _DEBUG";
#else
static char _id[] = "CAN API V3 for PEAK PCAN-Basic Interfaces, Version 3.2";
#endif
...
int can_interface(int handle, int *board, unsigned char *mode, void *param)
{
	...
	*mode  = can[handle].mode.byte;		// current operation mode
	if(param) {							// non-plug'n'play devices:
